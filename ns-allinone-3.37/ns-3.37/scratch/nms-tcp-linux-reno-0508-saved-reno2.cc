                ...
                Callback<void, uint32_t, uint32_t> CwndTrace)
{
    ...
    //source.SetAttribute("MaxBytes", UintegerValue(1048576000)); //100MB
...
main(int argc, char* argv[])
{
    ...
    //Config::SetDefault ("ns3::TcpSocket::ConnTimeout", TimeValue(Seconds (0.000240))); // syn timeout 5rtt 10*24=240us
    ...
    Config::SetDefault(qdiscTypeId + "::MaxSize", QueueSizeValue(QueueSize("1024p"))); //4MB
    ...
    // int weights[] = {10, 10, 10, 1, 1, 1, 1};
    int weights[] = {10, 10, 10, 1, 1, 1};
    double intervalT[] = {10.0, 10.0, 10.0, 10.05, 10.10, 10.15};
    int num_low_flows = 3;
    for (int i = 0; i < num_low_flows; i++)
    {
    ...
    for (int i = num_low_flows; i < num_clients; i++)
    {
        leftNodes.Get(i)->SetWeight(weights[i]);
        InstallBulkSend(leftNodes.Get(i), // node id 2
                        routerToRightIPAddress[0].GetAddress(1),
