...
SppifoQueueDisc::SppifoQueueDisc()
    : QueueDisc(SchedulingAlgorithm::STFQ, QueueDiscSizePolicy::NO_LIMITS, QueueSizeUnit::PACKETS)
{
...
SppifoQueueDisc::DoEnqueue(Ptr<QueueDiscItem> item)
{
    ...
    for (uint32_t i = GetNInternalQueues() - 1; i >= 0; i--)
    {
        if (rank >= m_bounds[i])
        {
            retval = GetQueueDiscClass(band)->GetQueueDisc()->Enqueue(item);
            if (retval)
            {
                band = i;
        ...
        else
        {
            if (i == 0)
            {
                retval = GetQueueDiscClass(i)->GetQueueDisc()->Enqueue(item);
                if (retval)
                {
                    band = i;
...
//     int32_t ret = ClassifyInSppifo(item);
...
SppifoQueueDisc::CheckConfig()
{
    ...
    {
        NS_LOG_ERROR("SppifoQueueDisc cannot have internal queues");
        return false;
    }
    ...
    // {
    //     NS_LOG_ERROR("SppifoQueueDisc needs at least 2 classes");
    //     return false;
    // }
